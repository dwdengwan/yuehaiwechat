...
export default {
  ...
  created(){
    ...
    if(that.getQueryVariable('groupOpenId')!==null){
      // 进入缴费页面时 localStorage无groupOpenId 但getQueryVariable可以获取到groupOpenId
      localStorage.setItem('groupOpenId',that.getQueryVariable('groupOpenId'))
      // 进入缴费页面时 无groupOpenId
      // this.fromWhatIn(query.type)
    }else if (that.getQueryVariable('groupOpenId') == null &&( localStorage.getItem('groupOpenId') == 'null'||localStorage.getItem('groupOpenId') == '') && query.type !== undefined) {
      // 进入缴费页面时 localStorage无groupOpenId getQueryVariable也获取不到groupOpenId
      this.getGroupOpenId()
    }else if (that.getQueryVariable('groupOpenId')!==null && query.type !== undefined) {
      // 进入缴费页面时 localStorage无groupOpenId 但getQueryVariable可以获取到groupOpenId
      localStorage.setItem('groupOpenId',that.getQueryVariable('groupOpenId'))
    }else if(localStorage.getItem('groupOpenId')!=='null' && that.getQueryVariable('groupOpenId')!==null && query.type !== undefined){
      this.fromWhatIn(query.type)
    }
  ...
  methods: {
    ...
    getGroupOpenId() {
      ...
      // alert(localStorage.getItem('userNo'))
      // alert(localStorage.getItem('groupCode'))
      ...
      this.$api.fetch({
        ...
        // alert(res)
        location.href = res;
        // alert('226r'+_this.getQueryVariable('groupOpenId'))
        localStorage.setItem('groupOpenId', _this.getQueryVariable('groupOpenId'));
        // alert('227r'+localStorage.getItem('groupOpenId'))
      })
    },
    goPay() {
    ...
    init() {
      // alert('r254'+localStorage.getItem('groupOpenId')) 
      let that = this;
      //that.getQueryVariable('groupOpenId') == null 1无gruopOpenId
      // if (that.getQueryVariable('groupOpenId') == null && this.$route.query.type === undefined) {
      //   localStorage.setItem('groupOpenId',that.getQueryVariable('groupOpenId'))
      // }else{
      //   this.getGroupOpenId()
      // }
      // type:1 快速缴费进入 2 多户页面进入 3 单户页面进入 都无groupOpenId
      let bool = this.$route.query.type == '1' || this.$route.query.type == '2' || this.$route.query.type == '3'; 
      // if (that.getQueryVariable('groupOpenId') !== null) {//有gruopOpenId
    ...
    comfirm() {
      ...
      // alert("r313"+localStorage.getItem('groupOpenId'));
      // Toast.success('缴费成功！')
      this.$api.fetch({
        params,//参数
