  ...
  methods: {
    ...
    init() {
      ...
      that.$api.fetch({
        ...
        if (res.length > 1) {// 多户页面
          ...
          localStorage.setItem('accountNo', res[0].accountNo);
          if (parseFloat(res[0].meterCount) > 1) {
            ...
            // 拿groupOpenId
            if (res[0].groupOpenId == '' && that.getQueryVariable('groupOpenId') == null) {
              that.getGroupOpenId('#/moreUser')
            ...
            that.$store.state.billData = res;
            console.log(res,that.$store.state.billData);
            that.$store.state.moreUserShow = 1;
    ...
    beforeClose(action, done) {
      ...
      if (action == 'confirm') {
        ...
        // alert('m323'+localStorage.getItem('groupOpenId'))
        this.$api.fetch({
          params,//参数
    ...
    goMoreAccount(item) {
      let that = this;
      localStorage.setItem('accountNo', item.accountNo);
    ...
    pay(item) {
      ...
      // alert('404more'+localStorage.getItem('groupOpenId'))
      // if (item.groupOpenId == '') {
      ...
      // }
      this.$router.push({ path: "/recharge",query: val })
