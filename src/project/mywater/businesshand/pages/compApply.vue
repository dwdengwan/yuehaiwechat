      ...
      <div class="content-padded">
        ...
        <div class="yh-body" @touchmove="touchmove($event)" @scroll="scroll($event)">
          <span class="span_content" v-show="contentShowA" @touchmove="touchmove($event)" @scroll="scroll($event)">
            ...
            <van-field class="yh-my-blue" required left-icon="location-o" v-model="tableData.groupName" @click="showAddressChuang" readonly label="所属地区：" placeholder="请选择地区" :right-icon="icon" clearable size="large" />
            <!-- @blur="blur(tableData.ctmName,'ctmName')"  -->
            <van-field class="yh-my-red" required left-icon="user-o" v-model="tableData.ctmName" :readonly="isReadyonly" clearable label="用水单位：" :placeholder="isReadyonly?'-':'请输入用水单位'" size="large" />
            <!-- @blur="blur(tableData.ctmIdCard,'ctmIdCard')"  -->
            <van-field class="yh-my-blue" required left-icon="idcard" v-model="tableData.ctmIdCard" :readonly="isReadyonly" clearable oninput="this.value=this.value.replace(/[^0-9Xx]+/,'');" label="经办人身份证号码：" :placeholder="isReadyonly?'-':'请输入身份证'" size="large" />
            <!-- @blur="blur(tableData.userAddress,'userAddress')"  -->
            <van-field class="yh-my-red" required left-icon="wap-home-o" v-model="tableData.userAddress" :readonly="isReadyonly" clearable label="用水地址：" :placeholder="isReadyonly?'-':'请输入用水地址'" size="large" />
            <!-- @blur="blur(tableData.operator,'operator')"  -->
            <van-field class="yh-my-blue" required left-icon="contact" v-model="tableData.operator" :readonly="isReadyonly" clearable label="经办人：" :placeholder="isReadyonly?'-':'请输入经办人'" size="large" />
            <!-- @blur="blur(tableData.operatorPhone,'operatorPhone')"  -->
            <van-field class="yh-my-red" required left-icon="phone-o" v-model="tableData.operatorPhone" :readonly="isReadyonly" clearable label="手机号码：" :placeholder="isReadyonly?'-':'请输入手机号码'" type="number" oninput="this.value=this.value.replace(/[^0-9]+/,'');" size="large" />
            <!-- @blur="blur(tableData.predictUser,'predictUser')"  -->
            <van-field class="yh-my-blue" required left-icon="friends-o" v-model="tableData.predictUser" :readonly="isReadyonly" clearable type="number" label="预估月用水人数：" :placeholder="isReadyonly?'-':'请输入用水人数'" size="large" />
            <!-- @blur="blur(tableData.applyCount,'applyCount')"  -->
            <!-- <van-field class="yh-my-red" required left-icon="orders-o" v-model="tableData.applyCount" :readonly="isReadyonly" clearable label="申请数量：" type="number" :placeholder="isReadyonly?'-':'请输入申请数量'" size="large" /> -->
            <!-- @blur="blur(tableData.predictWater,'predictWater')"  -->
            <van-field class="yh-my-red" left-icon="bar-chart-o" v-model="tableData.predictWater" :readonly="isReadyonly" clearable type="number" label="预估月用水量：" :placeholder="isReadyonly?'-':'请输入用水量'" size="large" />
            <van-field class="yh-my-blue" left-icon="comment-o" v-model="tableData.applyReason" :readonly="isReadyonly" clearable label="申请原因：" :placeholder="isReadyonly?'-':'请输入申请原因'" size="large" />
            <!-- <van-button type="info" class="next" @click="nextA" v-if="btnShow" style="margin-top: 10px;">下一步</van-button> -->
            ...
            <div class="yh-footer" v-if="btnShow">
              ...
              <div v-if="isShowInfo">
                ...
                <span class="kr-span">业务咨询电话：952525</span>
          ...
          <span class="span_content" v-show="contentShowB" @touchmove="touchmove($event)" @scroll="scroll($event)">
            ...
            <van-field class="yh-my-red" required left-icon="user-circle-o" v-model="bank.accountName" :readonly="isReadyonly" clearable label="开户名称：" :placeholder="isReadyonly?'-':'请输入开户名称'" size="large" />
            <!-- @blur="blur(bank.bankNo,'bankNo')"  -->
            <van-field class="yh-my-blue" required left-icon="contact" v-model="bank.bankNo" :readonly="isReadyonly" label="银行账户：" clearable :placeholder="isReadyonly?'-':'请输入银行账户'" size="large" />
            <!-- <div class="bottomBtn_content">
              <van-button type="info" class="next" style="float: left;margin-left: 3%" @click="goBackB" v-if="btnShow">上一步</van-button>
              <van-button type="info" class="next" @click="nextB" v-if="btnShow">下一步</van-button>
            </div> -->
            <div class="submit_box" v-if="btnShow">
          ...
          <span class="span_content" v-show="contentShowC" @touchmove="touchmove($event)" @scroll="scroll($event)">
            ...
            <van-field class="yh-my-blue" required left-icon="label-o" v-model="invoice.invoiceTypeName" readonly clearable label="发票类型：" :placeholder="isReadyonly?'-':'请选择发票类型'" size="large" :right-icon="icon" @click="selectRelation()" @click-right-icon="selectRelation()" />
            <!-- @blur="blur(invoice.unitName,'unitName')"  -->
            <van-field class="yh-my-red" required left-icon="shop-collect-o" v-model="invoice.unitName" :readonly="isReadyonly" clearable label="单位名称：" :placeholder="isReadyonly?'-':'请输入单位名称'" size="large" />
            <!-- @blur="blur(invoice.identifiNum,'identifiNum')"  -->
            <van-field class="yh-my-blue" required left-icon="user-circle-o" v-model="invoice.identifiNum" :readonly="isReadyonly" clearable label="纳税人识别号：" oninput="this.value=this.value.replace(/[^0-9.a-zA-Z]+/,'');" :placeholder="isReadyonly?'-':'请输入纳税人识别号'" size="large" />
            <!-- @blur="blur(invoice.mobile,'mobile')"  -->
            <van-field class="yh-my-red" required left-icon="phone-o" v-model="invoice.mobile" :readonly="isReadyonly" clearable label="联系电话：" :placeholder="isReadyonly?'-':'请输入联系电话'" type="number" oninput="this.value=this.value.replace(/[^0-9]+/,'');" size="large" />
            <!-- @blur="blur(invoice.invOpenBank,'invOpenBank')"  -->
            <van-field class="yh-my-blue" required left-icon="shop-o" v-model="invoice.invOpenBank" :readonly="isReadyonly" clearable label="开户银行：" :placeholder="isReadyonly?'-':'请选择开户银行'" size="large" />
            <!-- @blur="blur(invoice.invAccountNo,'invAccountNo')"  -->
            <van-field class="yh-my-red" required left-icon="contact" v-model="invoice.invAccountNo" :readonly="isReadyonly" clearable label="银行账户：" :placeholder="isReadyonly?'-':'请输入银行账户'" size="large" />
            <!-- @blur="blur(invoice.contactAddr,'contactAddr')"  -->
            <van-field class="yh-my-blue" required left-icon="location-o" v-model="invoice.contactAddr" :readonly="isReadyonly" clearable label="联系地址：" :placeholder="isReadyonly?'-':'请输入联系地址'" size="large" />
            <!-- <div class="bottomBtn_content">
            ...
            <div class="submit_box" v-if="btnShow">
              <van-button type="info" class="submit_btn" @click="goBackC" v-if="btnShow">上一步</van-button>
              <!-- v-if="!($route.query.isModify == 0)" -->
              <van-button type="info" class="submit_btn" @click="submit(1,'C')" v-if="isShowBtn">暂 存</van-button>
              <van-button type="info" class="submit_btn" @click="nextC" v-if="btnShow">下一步</van-button>
            </div>
          </span>

          <span class="span_content" v-show="contentShowD" @touchmove="touchmove($event)" @scroll="scroll($event)">
            <div v-if="PhotoShow1">
              <div class="upLoad_title">工商营业执照</div>
              <van-uploader v-model="fileList1" :max-count="1" :deletable='!isReadyonly' />
            </div>

            <div v-if="PhotoShow2">
              <div class="upLoad_title">法人身份证正面</div>
              <van-uploader v-model="fileList2" :max-count="1" :deletable='!isReadyonly' />
            </div>

            <div v-if="PhotoShow3">
              <div class="upLoad_title">法人身份证反面</div>
          ...
          <div class="submit_box" v-if="editShow">
            ...
            <van-button type="info" :disabled='isClick' class="submit_btn" @click="getImgBase64(1)" v-if="!btnShow && !($route.query.isModify == 0) && isDraft">暂 存</van-button>
            <van-button type="info" :disabled='isClick' class="submit_btn" @click="getImgBase64(0)" v-if="!btnShow && !($route.query.isModify == 0)">提交</van-button>
          </div>
          <div v-else>
...
export default {
  ...
  data() {
    return {
      ...
      isPeopleShow: 1,//0居民用水 1企业用水
      isQuery: false,//是否从进度查询进入
      message: {
        title: "企业用水申请",
        span1: "本业务适用于“企业用水报装”业务，",
        span2: "请您仔细阅读报装说明后填写相关信息",
        span4:'我们承诺：非经法律程序或经您的许可不会泄露您的个人信息',
        isShow: '',
      },
      applyType: '2',
      isShowInfo: false,
  ...
  methods: {
    blur(val, text) {//输入框失去焦点验证
      ...
      if (val === '') {
        ...
        } else if (text == 'applyCount') {
          // if ( parseInt(val) > 100000) {
          //   message = '申请数量超出范围!';
          //   Toast(message);
          //   this.tableData.applyCount = '';
          // }else
          if (val == 0) {
            message = '申请数量不能为0!';
            Toast(message);
        ...
        } else if (text == 'bankNo' && !/^([1-9]{1})(\d{14}|\d{15}|\d{16}|\d{17}|\d{18})$/.test(this.bank.bankNo)) {
          Toast('银行账户格式有误');
        } else if (text == 'invAccountNo' && !/^([1-9]{1})(\d{14}|\d{15}|\d{16}|\d{17}|\d{18})$/.test(this.invoice.invAccountNo)) {
          Toast('银行账户格式有误');
        }
      }
    },
    ...
    //     let item = {}
    //     item.isModify = '2'
    //     this.$router.push({ path: '/peopleApply', query: item })
    //   }
    // },
      ...
      axios(RequestObj).then(res => {
        if (res.status === 200) {
          if (res.data.code === 0) {
            ...
            // if (_this.$route.query.isModify == 2) {// 添加
      ...
      let params = {//接口入参
        "data": {
          ...
          'data': {
            ...
            // "applyCount": Number(this.tableData.applyCount),
            "accessory": [
              {
                "fileId": "",
                "type": "工商营业执照",
    ...
    nextA() {
      if (this.tableData.groupName === '') {
        ...
        Toast('用水单位未填');
        return
      } else if (this.tableData.operator === '') {
        Toast('经办人未填');
        return
      } else if (this.tableData.operatorPhone === '') {
        Toast('手机号码未填');
        return
      }
      else if (!/^1([38]\d|5[0-35-9]|7[3678])\d{8}$/.test(this.tableData.operatorPhone)) {
        Toast('手机号码格式有误');
        return
      }
      // else if (this.tableData.applyCount === '') {
      //   Toast('申请数量未填');
      //   return
      // } 
    ...
    nextB() {
      if (this.contentShowB) {
        ...
        else if (!/^([1-9]{1})(\d{14}|\d{15}|\d{16}|\d{17}|\d{18})$/.test(this.bank.bankNo)) {
          Toast('银行账户格式有误');
          return
        }
      }
    ...
    nextC() {
      if (this.contentShowC) {
        ...
        else if (!/^([1-9]{1})(\d{14}|\d{15}|\d{16}|\d{17}|\d{18})$/.test(this.invoice.invAccountNo)) {
          Toast('银行账户格式有误');
          return
        }
        else {
    ...
    testC() {
      if (this.invoice.mobile !== '' && !/^1([38]\d|5[0-35-9]|7[3678])\d{8}$/.test(this.invoice.mobile)) {
        Toast('联系电话格式有误');
        return this.isTestErr = false;
      } else if (this.invoice.invAccountNo !== '' && !/^([1-9]{1})(\d{14}|\d{15}|\d{16}|\d{17}|\d{18})$/.test(this.invoice.invAccountNo)) {
        Toast('银行账户格式有误');
        return this.isTestErr = false;
      }
      this.isTestErr = true;
